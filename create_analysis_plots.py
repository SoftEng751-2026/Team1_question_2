...
def plot_speedup(stats, sequential_name, parallel_name, lang_label, title, filename):
    ...
    # FIXED: Added a 4th color color mapping for 4096
...
plot_ratio(gol, ('cpp_sequential', 'java_sequential'), ('cpp_optimized', 'java_optimized'),
           'Game of Life — C++ vs Java Performance Ratio', 'ratio_gol.png')
plot_ratio(heat, ('HeatDiffusion_Cpp_Sequential', 'HeatDiffusion_Java_Sequential'),
           ('HeatDiffusion_Cpp_OMP', 'HeatDiffusion_Java_ForkJoin'),
           'Heat Diffusion — C++ vs Java Performance Ratio', 'ratio_heat.png')
...
def plot_speedup_combined(stats, cpp_ser, cpp_par, java_ser, java_par, title, filename):
    sizes = stats[['rows', 'cols']].drop_duplicates().values
    num_sizes = len(sizes)
    # FIXED: Grid dynamically scales to 4 subplots to prevent any array out of bounds clipping
...
print("All 4096 plots successfully refreshed!")
